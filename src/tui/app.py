...
from tui.widgets import HannaCommandBar
...
class HannaTUIApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="command-bar"):
            ...
            yield HannaCommandBar(id="command-input")
    ...
    def _focus_command_input(self) -> None:
        if not self.is_mounted:
            return
        self.query_one("#command-input", HannaCommandBar).focus()
    ...
    def _handle_editor_result(self, result: Optional[Dict[str, str]]) -> None:
        if not result:
    ...
    @on(HannaCommandBar.CommandSubmitted)
    def handle_command_submitted(self, message: HannaCommandBar.CommandSubmitted) -> None:
