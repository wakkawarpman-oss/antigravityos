...
class SessionState:
    ...
    latest_result: Optional[RunResult] = None
...
def build_default_session_state(
    ...
    # Perform hydration if DB exists
    db_file = Path(db_path or str(DEFAULT_DB_PATH))
    if db_file.exists():
        hydrate_session_from_repo(session, str(db_file))
    
    return session


def hydrate_session_from_repo(state: SessionState, db_path: str) -> None:
    """Load historical data from DiscoveryRepository into the current SessionState."""
    repo = DiscoveryRepository(db_path)
    try:
        all_obs = repo.get_all_observables()
        if not all_obs:
            return

        # Convert to ObservableState
        historical_states: list[ObservableState] = []
        for obs in all_obs:
            historical_states.append(
                ObservableState(
                    ...
                    confidence=1.0 if obs.tier == "confirmed" else (0.7 if obs.tier == "probable" else 0.4),
                    status=obs.tier
                )
            )
        
        # Merge with current (seed) observables
        state.observables = _dedup_observables(state.observables + historical_states)
        ...
        state.activity.append(
            ActivityEvent(
                level="info", 
                text=f"Hydrated state from {db_path}: {stats['observables']} observables, {stats['entity_links']} links.", 
                timestamp=datetime.now().isoformat(timespec="seconds")
            )
        )
        
        # Update target name if original target exists
        for obs in all_obs:
            if obs.is_original_target:
                state.target.label = obs.value
                break
...
def set_phase(state: SessionState, phase: str, detail: Optional[str] = None) -> None:
...
def update_phase_counters(state: SessionState, phase: str, counters: Dict[str, Union[str, int, float]]) -> None:
...
def _initial_module_names(resolved_modules: list[str], manual_module: Optional[str]) -> list[str]:
    if manual_module:
