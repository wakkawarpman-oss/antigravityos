...
class ReconAdapter(ABC):
    ...
    def __init__(self, proxy: Optional[str] = None, timeout: float = 10.0, leak_dir: Optional[str] = None):
    ...
    async def _fetch_async(self, url: str, headers: Optional[Dict[str, str]] = None) -> Tuple[int, str]:
        """Async fetch through proxy using thread pool isolation."""
        return await asyncio.to_thread(self._fetch, url, headers)

    async def _post_async(self, url: str, data: Dict[str, Any], headers: Optional[Dict[str, str]] = None) -> Tuple[int, str]:
        """Async post through proxy using thread pool isolation."""
        return await asyncio.to_thread(self._post, url, data, headers)
    ...
    def search(
        ...
        """Run search and return findings (Sync legacy)."""
        ...
    ...
    async def search_async(
        ...
        """
        Run search asynchronously and yield hits one-by-one.
        Default implementation wraps sync search().
        """
        # Default behavior for non-refactored adapters: run sync and yield results
        hits = await asyncio.to_thread(self.search, target_name, known_phones, known_usernames)
        for hit in hits:
            yield hit
