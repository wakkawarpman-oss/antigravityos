...
class ReportRenderer:
    ...
    def get_runs_dir(self) -> Path:
        db_file = self.engine.repo.get_database_path()
        if db_file:
    ...
    @staticmethod
    def lane_from_source_tool(source_tool: str, lane_registry: Dict[str, str]) -> Optional[str]:
        if not source_tool:
            return None
    ...
    def load_latest_deep_recon_report(self) -> Optional[Dict[str, Any]]:
    ...
    def build_lane_summary(self, primary: Any) -> Dict[str, Any]:
        ...
        observable_rows = self.engine.repo.db.execute(
    ...
    def redact_value(self, value: str, obs_type: Optional[str] = None, mode: str = "shareable") -> str:
    ...
    def render_graph_report(self, output_path: Union[str, Path, None] = None, redaction_mode: str = "shareable") -> str:
        ...
        if primary:
            ...
            edges = self.engine.repo.get_edges()
        ...
        if primary:
            for url in primary.profile_urls[:50]:
                ...
                url_status = self.engine.repo.get_profile_status(url)
                url_status = url_status[0] if url_status else "unchecked"
                status_badge = {
        ...
        rejected_rows = self.engine.repo.get_rejected_targets(20)
        rejected_html = ""
