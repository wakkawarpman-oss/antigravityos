...
def _resolve_proxy(args: argparse.Namespace) -> Optional[str]:
    """Resolve --tor / --proxy into a single proxy string.

    Raises if both are provided (per OPSEC_RUNBOOK.md).
    """
...
def _parse_export_formats(value: Optional[str]) -> List[str]:
    if not value:
        return []
...
def _resolve_export_formats(export_formats_value: Optional[str], metadata_file: Optional[str]) -> List[str]:
...
def _emit_json_payload(payload: Dict[str, object], output_file: Optional[str] = None) -> None:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ch.add_argument("--tor", action="store_true", help="Route through Tor (socks5h://127.0.0.1:9050)")
    ...
    ag.add_argument("--tor", action="store_true", help="Route through Tor (socks5h://127.0.0.1:9050)")
    ...
    mn.add_argument("--tor", action="store_true", help="Route through Tor (socks5h://127.0.0.1:9050)")
    ...
    tui.add_argument("--tor", action="store_true", help="Route through Tor (socks5h://127.0.0.1:9050)")
...
def _infer_nuclei_profile(module_tokens: List[str]) -> Optional[str]:
...
def _configure_nuclei_profile(explicit_profile: Optional[str], module_tokens: List[str]) -> None:
